...
$sidc=@$sidsql['c'];
$sidj=@$sidsql['j'];
$all=$DB->get_row("SELECT * from app_singlerouter WHERE user='$key' ");
if(!$all){die("[失败]管理员还没设置单路由模式网关地址");}
$ip=$all['ip'];
$all=$DB->get_row("SELECT * from app_account WHERE user='$key' ");
if(!$all){die("[失败]管理员没有设置单路由模式账号密码");}
$sid=@$_GET['sid'];
$timei=@date('i');
...
$post = '{"username":"' . $account . '","passwd":"' . md5($passwd) . '","pass":"' . base64_encode('salt_11' . $passwd) . '","remember_password":false}';
...
if ($ErrMsg == 'Success') {
    ...
    $headers = [
        ...
        'Cookie:username=' . $account . '; login=1;' . $cookie
    ];
